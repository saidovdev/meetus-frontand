...
export const add_contact=createAsyncThunk(
    ...
    async({telegram,whatsApp,instagram,phone,email},{rejectWithValue,getState})=>{
        try {
            console.log("ishladi")
            const response=await api.post('/user/contact',{telegram,whatsApp,instagram,phone,email})
            toast.success('Contact added succesfully')
            return response.data
        } catch (error) {
            console.log("Error ocured while posting contact",error.status,error.message)
